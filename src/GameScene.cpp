...
static SDL_Texture* LoadScaledTexture(
    ...
    if (tex) {
        SDL_SetTextureScaleMode(
            tex, (tw <= 0 || th <= 0) ? SDL_SCALEMODE_LINEAR : SDL_SCALEMODE_NEAREST);
    }
    return tex;
}

SDL_Texture* GameScene::GetCachedTexture(
    SDL_Renderer* ren, const std::string& path, int w, int h, int rot) {
...
void GameScene::PreloadRawSurfaces(const Level& level) {
    ...
    for (auto& f : futures)
        for (auto& [p, s] : f.get())
            if (s)
                sRawSurfaceCache.emplace(p, s);

    std::print("PreloadRawSurfaces: {} assets across {} threads\n",
               pending.size(),
               std::min(hw, (int)pending.size()));
}
...
void GameScene::Load(Window& window) {
    ...
    auto loadSlot = [&](PlayerAnimSlot     slot,
        ...
        if (useProfile && profile.HasSlot(slot)) {
            ...
            if (!pngs.empty()) {
                ...
                // Explicit sorted paths — no prefix filtering, so slot reuse works
                // correctly.
...

    {
        ...
        gameOverText = std::make_unique<Text>(
            "Game Over!", SDL_Color{255, 0, 0, 255}, cx - goSize.x / 2, cy - 80, 64);
        ...
        retryBtnText   = std::make_unique<Text>(
            "Retry", SDL_Color{0, 0, 0, 255}, btnTxtPos.x, btnTxtPos.y, 32);
            ...
            for (int i = 0; i < PLAYER_ANIM_SLOT_COUNT; ++i) {
                ...
                for (int fi = 0; fi < (int)slot.sfx.size(); ++fi) {
                    ...
                    std::print("[Audio] Loading slot {} file {} sfxId='{}' path='{}'\n",
                               ...
                               slot.sfx[fi].path);
                    if (!sfxId.empty()) {
                        bool ok = Audio()->Sfx().LoadTrimmed(sfxId,
                                                             slot.sfx[fi].path,
                                                             slot.sfx[fi].trimStart,
                                                             slot.sfx[fi].trimEnd);
                        std::print("[Audio]   -> {}\n", ok ? "OK" : "FAILED");
                    }
                }
...
void GameScene::Update(float dt) {
    ...
    auto tileQueryFn = [&](float px, float pyBottom, float& tileTopY) -> bool {
        bool hit = false;
        mTileGrid.Query(px - 2.f, pyBottom - 2.f, 4.f, 4.f, [&](entt::entity te) {
            if (hit)
                return;
            if (reg.all_of<HazardTag>(te) || reg.all_of<LadderTag>(te))
                return;
            if (!reg.all_of<Transform, Collider>(te))
                return;
            const auto& tt = reg.get<Transform>(te);
            const auto& tc = reg.get<Collider>(te);
            if (px >= tt.x && px <= tt.x + tc.w && pyBottom >= tt.y &&
                pyBottom <= tt.y + tc.h) {
                tileTopY = tt.y;
                hit      = true;
            }
    ...
    auto tileMoveQueryFn = [&](float probeX, float probeY, float& dvx, float& dvy) -> bool {
        bool found = false;
        mTileGrid.Query(probeX - 2.f, probeY - 2.f, 4.f, 4.f, [&](entt::entity te) {
            if (found)
                return;
            if (!reg.all_of<MovingPlatformTag, MovingPlatformState, Transform, Collider>(te))
                return;
            const auto& tt  = reg.get<Transform>(te);
            const auto& tc  = reg.get<Collider>(te);
            const auto& mps = reg.get<MovingPlatformState>(te);
            if (probeX >= tt.x && probeX <= tt.x + tc.w && probeY >= tt.y &&
                probeY <= tt.y + tc.h) {
                dvx   = mps.vx;
                dvy   = mps.vy;
                found = true;
    ...
    if (mPlayerDying) {
        ...
        bool animDone = false;
        reg.view<PlayerTag, AnimationState, DeadTag>().each([&](const AnimationState& anim) {
    ...
    MovementSystem(
        reg, dt, mWindow->GetWidth(), mLevelW, mCachedPad, &mTileGrid, mCachedPad2);
    BoundsSystem(reg,
                 dt,
                 mWindow->GetWidth(),
                 mWindow->GetHeight(),
    ...
    auto enemySfxVolume = [&](float ex, float ey, float baseVol) -> float {
        ...
        if (minDist <= FULL_DIST)
            return baseVol;
        if (minDist >= SILENT_DIST)
            return 0.0f;
    ...
    if (Audio() && Audio()->IsReady() && !prevEnemySnap.empty()) {
        auto ev = reg.view<EnemyTag,
                           EnemyAnimData,
                           AnimationState,
                           Renderable,
                           Transform,
                           Collider>(entt::exclude<DeadTag>);
        ev.each([&](entt::entity          e,
                    EnemyAnimData&        ead,
                    const AnimationState& anim,
                    const Renderable&     r,
            ...
            if (pit == prevEnemySnap.end())
                return;
            if (r.sheet != pit->second.sheet)
                return; // sheet changed — handled below
            if (!anim.looping)
                return;
            ...
            int slotIdx = -1;
            if (r.sheet == ead.moveSheet)
                slotIdx = 1;
            else if (r.sheet == ead.idleSheet)
                slotIdx = 0;
            if (slotIdx < 0)
                return;

            auto& ss = ead.slotSfx[slotIdx];
            if ((int)ss.files.size() <= 1)
                return; // single file loops via SDL mixer
    ...
    // Recover enemies from hurt/attack animation back to move animation
    {
        ...
        hurtView.each([&](entt::entity         e,
                          const EnemyAnimData& ead,
                          AnimationState&      anim,
                          Renderable&          r) {
            if (anim.looping)
                return;
        ...
        auto atkView =
            reg.view<EnemyTag, EnemyAttackState, EnemyAnimData, AnimationState, Renderable>(
                entt::exclude<DeadTag>);
        atkView.each([&](entt::entity         e,
                         EnemyAttackState&    eas,
                         const EnemyAnimData& ead,
                         AnimationState&      anim,
                         Renderable&          r) {
    ...
    // A consumed set prevents the same item being given to multiple players.
    {
        ...
        pv.each([&](entt::entity playerEnt, const Transform& pt, const Collider& pc) {
            ...
            puv.each([&](entt::entity e,
                         ...
                         const Transform& t,
                         const Collider&  c) {
                if (std::find(consumed.begin(), consumed.end(), e) != consumed.end())
                    return;
            ...
            for (entt::entity e : toConsume) {
                ...
                if (pu.type == PowerUpType::Teleport) {
                    if (reg.all_of<TeleportDestTag>(e))
                        continue;
                    ...
                    if (mMultiplayerActive) {
                        ...
                        // Teleport every player to the destination, spacing them
                        // side-by-side.
                        int  grp        = pu.teleportGroup;
                        bool teleported = false;
                        auto destView   = reg.view<TeleportDestTag, Transform>();
                        destView.each([&](entt::entity           de,
                                          const TeleportDestTag& dest,
                                          const Transform&       dt) {
                            if (teleported || dest.group != grp)
                                return;
                            int dw = 0, dh = 0;
                        ...
                        if (reg.all_of<TeleportCooldown>(playerEnt))
                            continue;
                        int  grp        = pu.teleportGroup;
                        bool teleported = false;
                        auto destView   = reg.view<TeleportDestTag, Transform>();
    ...
    for (entt::entity e : floatResult.enemiesKilledByFloat) {
        if (!reg.valid(e) || reg.all_of<DeadTag>(e))
            continue;
    ...
    if (Audio() && Audio()->IsReady() && !prevEnemySnap.empty()) {
        ...
        ev.each([&](entt::entity          e,
            ...
            if (r.sheet == pit->second.sheet && !retrigger)
                return;
            int slotIdx = -1;
            if (r.sheet == ead.idleSheet)
                slotIdx = 0;
            else if (r.sheet == ead.moveSheet)
                slotIdx = 1;
            else if (r.sheet == ead.attackSheet)
                slotIdx = 2;
            else if (r.sheet == ead.hurtSheet)
                slotIdx = 3;
            else if (r.sheet == ead.deadSheet)
                slotIdx = 4;
            if (slotIdx < 0 || ead.slotSfx[slotIdx].files.empty())
                return;
            auto&       ss = ead.slotSfx[slotIdx];
            const auto& fi = ss.files[ss.nextIdx];
            float vol = enemySfxVolume(et.x + ec.w * 0.5f, et.y + ec.h * 0.5f, fi.volume);
    ...
    auto killPlayer = [&](entt::entity pe) {
        if (!reg.valid(pe) || reg.all_of<DeadTag>(pe))
            return;

        // Stop movement
        if (auto* v = reg.try_get<Velocity>(pe)) {
            v->dx = 0.0f;
            v->dy = 0.0f;
        }
        ...
        if (auto* pt = reg.try_get<Transform>(pe)) {
            if (auto* pc = reg.try_get<Collider>(pe)) {
                emitBlood({pt->x + pc->w * 0.5f,
                           pt->y + pc->h * 0.5f,
                           0.f,
                           -1.f,
                           BloodEventType::PlayerDeath,
                           1.0f});
            }
        }
        ...
        if (!mPlayerDying) {
            bool anyAlive = false;
            reg.view<PlayerTag>(entt::exclude<DeadTag>).each([&](entt::entity) {
                anyAlive = true;
            });
            if (!anyAlive) {
                mPlayerDying    = true;
    ...
    if (collision.slashHits > 0) {
        ...
        pv.each([&](const Velocity& v) { punchDir = (v.dx >= 0.0f) ? 1.0f : -1.0f; });
        float progress  = 1.0f - collision.lowestHitHpFrac;
    ...
    for (entt::entity e : collision.actionTilesTriggered) {
            ...
            if (rend) {
                ...
                if (bestPe != entt::null) {
                    ...
                    if (as) {
                        ...
                        as->orbitRadius = std::max(
                            as->orbitRadius, (float)std::max(se.renderW, se.renderH) * 1.2f);
                    } else {
                        ActiveShield newAs;
        ...
        // Goals with destroy animations are collected when the anim finishes (see
        // DestroyAnimTag cleanup).
                ...
                if (!frameTex.empty() && frameTex[0]) {
                    ...
                    if (reg.all_of<Transform>(e)) {
                        ...
                        reg.emplace<Renderable>(
                            e, fvec[0], std::move(animRects), false, tileW, tileH);
                    }
        ...
        hView.each([&](entt::entity        playerEnt,
            ...
            bool            thisPlayerOnHazard = false;
            {
                ...
                hazView.each([&](entt::entity he, const Transform& ht, const Collider& hc) {
                    ...
                    if (pt.x < hx + hc.w + TOUCH && pt.x + pc.w > hx - TOUCH &&
                        pt.y < hy + hc.h + TOUCH && pt.y + pc.h > hy - TOUCH)
    ...
    if (Audio() && Audio()->IsReady()) {
        ...
        pview.each([&](entt::entity e, const AnimationState& anim) {
            ...
            bool animRestarted =
                !animChanged && !anim.looping && anim.currentFrame == 0 && prevFrame > 0;
            if (!animChanged && !animRestarted)
                return;

            std::print(
                "[Audio] Player {:x} anim {}: {} -> {} (frames={} fps={:.1f} loop={})\n",
                (uint32_t)e,
    ...
    if (!mPlayerDying && !gameOver) {
        ...
        if (!turretTargets.empty()) {
            ...
            sv.each([&](entt::entity      turretEnt,
                ...
                if (nearestDist > sh.range || ss.cooldownLeft > 0.f)
                    return;
    ...
    bool bulletKilledPlayer = false;
    {
        ...
        bv.each([&](entt::entity be, BulletTag& bt, Transform& btr) {
            ...
            // Hit solid tiles via spatial grid (skip source turret; player bullets pass
            // through hazards & floats)
            bool hitTile = false;
            mTileGrid.Query(btr.x - 4.f, btr.y - 4.f, 16.f, 16.f, [&](entt::entity te) {
                ...
                if (!reg.valid(te))
                    return;
                if (te == bt.sourceTurret)
                    return;
                if (!reg.all_of<TileTag>(te))
                    return;
                if (bt.playerOwned && reg.all_of<HazardTag>(te))
                    return;
                if (bt.playerOwned && reg.all_of<FloatTag>(te))
                    return;
            ...
            // Hit shield (absorbs bullet)
            {
                ...
                shv.each([&](const Transform& pt, const Collider& pc, ActiveShield& as) {
                    ...
                    for (int i = 0; i < n && !blocked; ++i) {
                        ...
                        // spinAngle is a visual-only texture rotation — does not shift orbit
                        // position
                        ...
                        if (SDL_HasRectIntersectionFloat(&bulletR, &shieldR)) {
                            ...
                            if (std::abs(lx) > CORNER_THRESH &&
                                std::abs(ly) > CORNER_THRESH) {
                    ...
                    if (SDL_HasRectIntersectionFloat(&bulletR, &playerR)) {
                        ...
                        if (auto* hp = reg.try_get<Health>(pe)) {
                            ...
                            bulletBloodIntensity = std::min(
                                std::clamp(0.4f + dmgFrac * 3.0f, 0.4f, 3.0f) + killBonus,
                                3.0f);
                        }
                        ...
                        // Blood spurts from the player scaled by how much damage the bullet
                        // dealt
                        emitBlood({pCX,
                                   pCY,
                                   bt.dx,
            ...
            // Hit enemies
            auto ev =
                reg.view<EnemyTag, Transform, Collider, Health>(entt::exclude<DeadTag>);
            for (auto ee : ev) {
                ...
                if (SDL_HasRectIntersectionFloat(&bulletR, &enemyR)) {
                    ...
                    if (eh.current <= 0.f) {
                        ...
                        float killIntensity = std::min(
                            std::clamp(0.4f + bulletDmgFrac * 3.0f, 0.4f, 3.0f) + 0.5f,
                            3.0f);
                        emitBlood({hitX,
                                   hitY,
                                   bt.dx,
                                   bt.dy,
                                   BloodEventType::BulletKillEnemy,
                                   killIntensity});
                        ...
                        float woundIntensity =
                            std::clamp(0.4f + bulletDmgFrac * 3.0f, 0.4f, 3.0f);
                        emitBlood({hitX,
                                   hitY,
                                   bt.dx,
                                   bt.dy,
                                   BloodEventType::BulletHitEnemy,
                                   woundIntensity});
            ...
            if (bt.playerOwned) {
                ...
                mTileGrid.Query(btr.x - 4.f, btr.y - 4.f, 16.f, 16.f, [&](entt::entity he) {
                    if (hitHazard != entt::null)
                        return;
                    if (!reg.valid(he) || !reg.all_of<HazardTag>(he))
                        return;
                ...
                if (hitHazard != entt::null) {
                    auto he = hitHazard;
                    if (reg.all_of<HazardTag>(he))
                        reg.remove<HazardTag>(he);
                    if (reg.all_of<TileTag>(he))
                        reg.remove<TileTag>(he);
                    if (reg.all_of<Collider>(he))
                        reg.remove<Collider>(he);
                    ...
                    if (atag && !atag->destroyAnimPath.empty()) {
                        ...
                        if (LoadAnimatedTileDef(atag->destroyAnimPath, def) &&
                            ...
                            if (!frameTex.empty() && frameTex[0]) {
                                ...
                                tr.y         = tileCY - animRenderH * 0.5f;
                                if (reg.all_of<Renderable>(he))
                                    reg.remove<Renderable>(he);
                                reg.emplace<Renderable>(he,
                                                        fvec[0],
                                                        std::move(animRects),
                                                        false,
                                                        animRenderW,
                                                        animRenderH);
                                if (reg.all_of<AnimationState>(he))
                                    reg.remove<AnimationState>(he);
                                reg.emplace<AnimationState>(
                    ...
                    if (!playedAnim) {
                        auto it = std::find(
                            mSortedTileRenderList.begin(), mSortedTileRenderList.end(), he);
                        if (it != mSortedTileRenderList.end())
                            mSortedTileRenderList.erase(it);
                        reg.destroy(he);
                    }
                ...
                mTileGrid.Query(btr.x - 4.f, btr.y - 4.f, 16.f, 16.f, [&](entt::entity ae) {
                    if (hitAction != entt::null)
                        return;
                    if (!reg.valid(ae) || !reg.all_of<ActionTag>(ae))
                        return;
                    if (reg.all_of<HazardTag>(ae))
                        return;
                    if (!reg.all_of<Collider>(ae))
                        return;
    ...
    // --- Shield orbit ---
    {
        ...
        shv.each([&](entt::entity       pe,
                     const Transform&   pt,
                     const Collider&    pc,
                     ActiveShield&      as,
                     const PlayerIndex& pi) {
            ...
            constexpr float SPIN_DECAY = 10.0f; // rad/s² deceleration — low = spins longer
    ...
    {
        auto tpv =
            reg.view<PlayerTag, Transform, Collider, ActiveTurretPowerUp, PlayerIndex>();
        tpv.each([&](entt::entity         pe,
                     const Transform&     pt,
                     const Collider&      pc,
                     ActiveTurretPowerUp& tp,
                     const PlayerIndex&   pi) {
...

    {
        ...
        if (pcenters.size() >= 2) {
            ...
            mCamera.UpdateMulti(pcenters[0].cx,
                                pcenters[0].cy,
                                pcenters[1].cx,
                                pcenters[1].cy,
                                mWindow->GetWidth(),
    ...
    mBloodParticles.Update(
        ...
        [&](float px, float pyBottom, float& tileTopY) -> bool {
            ...
            mTileGrid.Query(px - 2.f, pyBottom - 2.f, 4.f, 4.f, [&](entt::entity te) {
                ...
                if (!reg.valid(te))
                    return;
                if (!reg.all_of<TileTag, Transform, Collider>(te))
                    return;
                if (reg.all_of<HazardTag>(te))
                    return;
                if (reg.all_of<LadderTag>(te))
                    return;
                ...
                if (px >= tx && px <= tx + tc.w && pyBottom >= ty && pyBottom <= ty + tc.h) {
                    if (ty < bestTop) {
                        bestTop = ty;
                        hit     = true;
                    }
...
void GameScene::Render(Window& window, float alpha) {
    ...
    for (size_t i = 0; i < mParallaxImages.size(); ++i)
        mParallaxImages[i]->RenderScrolling(
            ren, mCamera.x * mParallaxFactors[i], (float)mLevelW);
    ...
    if (levelComplete) {
        RenderSystem(reg,
                     ...
                     W,
                     H,
                     &mSortedTileRenderList,
                     alpha,
                     &mSortedFrontPropList);
        ...
        if (mDebugHitboxes) {
...

            {
                ...
                pv.each([&](entt::entity          pe,
                    ...
                    std::string info = std::string(animName) + " " + std::to_string(c.w) +
                                       "x" + std::to_string(c.h);
                    ...
                    if (const auto* roff = reg.try_get<RenderOffset>(pe)) {
                        ...
                        Text offLbl(offStr, SDL_Color{0, 200, 200, 200}, sx + 2, sy + 2, 9);
                        offLbl.Render(ren);
                    }
                });
            }
...
void GameScene::BuildPauseUI(int W, int H) {
    ...
    mPauseTitleLbl = std::make_unique<Text>(
        "PAUSED", SDL_Color{255, 215, 0, 255}, cx - titleSize.x / 2, cy - 140, 36);
...
void GameScene::RenderShield(SDL_Renderer* ren, int W, int H) {
    ...
    sv.each([&](const Transform& pt, const Collider& pc, const ActiveShield& as) {
        ...
        for (auto& se : as.shields)
            minRemaining = std::min(minRemaining, se.remaining);
        ...
        for (int i = 0; i < n; ++i) {
            ...
            if (se.shakeTimer > 0.f) {
                ...
                shakeY =
                    amp * std::cos(se.shakeTimer * 109.f); // different freq → elliptical
            }
            ...
            if (screenX + se.renderW < 0 || screenX > W || screenY + se.renderH < 0 ||
                screenY > H)
                continue;
            ...
            if (se.tex) {
                ...
                SDL_RenderTextureRotated(
                    ren, se.tex, nullptr, &dst, spinDeg, nullptr, SDL_FLIP_NONE);
...
void GameScene::SpawnPlayer2() {
    ...
    if (hasP2Profile) {
        ...
        auto p2GetFrames = [&](SpriteSheet*       sheet,
                               PlayerAnimSlot     slot,
                               const std::string& knightKey) -> std::vector<SDL_Rect> {
            if (p2Prof.HasSlot(slot))
                return sheet->GetAnimation("");
        ...
        SpriteSheet* walkSh =
            loadP2Slot(PlayerAnimSlot::Walk, "Walking", "0_Knight_Walking_", 24);
        SpriteSheet* jumpSh =
            loadP2Slot(PlayerAnimSlot::Jump, "Jump Start", "0_Knight_Jump Start_", 6);
        SpriteSheet* hurtSh = loadP2Slot(PlayerAnimSlot::Hurt, "Hurt", "0_Knight_Hurt_", 12);
        SpriteSheet* slideSh =
            loadP2Slot(PlayerAnimSlot::Crouch, "Sliding", "0_Knight_Sliding_", 6);
        SpriteSheet* fallSh =
            loadP2Slot(PlayerAnimSlot::Fall, "Falling Down", "0_Knight_Falling Down_", 6);
        SpriteSheet* slashSh =
            loadP2Slot(PlayerAnimSlot::Slash, "Slashing", "0_Knight_Slashing_", 12);
        SpriteSheet* deathSh =
            loadP2Slot(PlayerAnimSlot::Death, "Hurt", "0_Knight_Hurt_", 12);
        ...
        if (!p2IdleF.empty()) {
            if (!p2Prof.HasSlot(PlayerAnimSlot::Walk) && p2WalkF.empty())
                p2WalkF = p2IdleF;
            if (!p2Prof.HasSlot(PlayerAnimSlot::Jump) && p2JumpF.empty())
                p2JumpF = p2IdleF;
            if (!p2Prof.HasSlot(PlayerAnimSlot::Hurt) && p2HurtF.empty())
                p2HurtF = p2IdleF;
            if (!p2Prof.HasSlot(PlayerAnimSlot::Crouch) && p2DuckF.empty())
                p2DuckF = p2IdleF;
            if (!p2Prof.HasSlot(PlayerAnimSlot::Fall) && p2FrontF.empty())
                p2FrontF = p2IdleF;
            if (!p2Prof.HasSlot(PlayerAnimSlot::Slash) && p2SlashF.empty())
                p2SlashF = p2IdleF;
            if (!p2Prof.HasSlot(PlayerAnimSlot::Death) && p2DeathF.empty())
                p2DeathF = p2IdleF;
        }
    ...
    reg.emplace<AnimationSet>(p2,
                              AnimationSet{
                                  ...
                                  .hurtSheet  = p2HurtTex,
                                  .duck       = p2DuckF,
                                  .duckSheet  = p2SlideTex,
                                  .front      = p2FrontF,
                                  .frontSheet = p2FallTex,
                                  .slash      = p2SlashF,
                                  .slashSheet = p2SlashTex,
                                  .death      = p2DeathF,
                                  .deathSheet = p2DeathTex,
                              });

    std::print("SpawnPlayer2: P2 entity created at ({:.0f}, {:.0f}) profile='{}'\n",
               spawnX,
               spawnY,
               mP2ProfilePath.empty() ? "(default)" : mP2ProfilePath);
}
...
void GameScene::Spawn() {
    ...
    goalText    = std::make_unique<Text>("Goals: 0", SDL_Color{0, 255, 120, 255}, 0, 0, 16);
    ...
    bool          hasSpawnProfile =
        !mProfilePath.empty() && LoadPlayerProfile(mProfilePath, spawnProfile);
    ...
    reg.emplace<Renderable>(player,
                            knightIdleSheet->GetTexture(),
                            idleFrames,
                            false,
                            mPlayerSpriteW,
                            mPlayerSpriteH);
    ...
    reg.emplace<RenderOffset>(player, pROffX, pROffY);
    {
        ...
        const AnimHitbox& crouchHB = hasSpawnProfile
                                         ? spawnProfile.Slot(PlayerAnimSlot::Crouch).hitbox
                                         : AnimHitbox{};
        if (!crouchHB.IsDefault()) {
            base.duckW     = crouchHB.w;
            base.duckH     = crouchHB.h;
    ...
    // so resolveSheet uses slot capability instead of pointer identity to pick the right
    // texture.
    ...
    reg.emplace<AnimationSet>(
        ...
        AnimationSet{
            ...
            .duck      = duckFrames,
            .duckSheet =
                resolveSheet(knightSlideSheet->GetTexture(), PlayerAnimSlot::Crouch),
            ...
            .slash      = slashFrames,
            .slashSheet =
                resolveSheet(knightSlashSheet->GetTexture(), PlayerAnimSlot::Slash),
    ...
    for (const auto& ts : mLevel.tiles) {
        if (IsAnimatedTile(ts.imagePath)) {
            ...
            // Native-res frames (0,0) — GPU scales at render time. CPU pre-scale destroyed
            // lava detail.
            std::vector<SDL_Texture*> frameTex;
            for (const auto& fp : def.framePaths)
            ...
            reg.emplace<Renderable>(
                tile, frameTex[0], std::move(frameRects), false, animW, animH);
                ...
                if (puType != PowerUpType::None) {
                    ...
                    if (puType == PowerUpType::Teleport) {
                        if (ts.powerUp->teleportDest)
                            ...
                            reg.emplace<TeleportEntranceTag>(tile,
                                                             ts.powerUp->teleportGroup);
                    }
                }
            }
            if (ts.HasShooter()) {
    ...
    auto getEnemyTypeCache =
        [&](const std::string& typeName) -> std::shared_ptr<EnemyTypeCache> {
        auto it = enemyTypeCache.find(typeName);
        if (it != enemyTypeCache.end())
            return it->second;
        ...
        auto loadSlot =
            [&](EnemyAnimSlot slot) -> std::pair<SpriteSheet*, std::vector<SDL_Rect>> {
            if (!prof.HasSlot(slot))
                return {nullptr, {}};
            ...
            if (fs::is_directory(dir, ec) && !ec) {
                for (const auto& e : fs::directory_iterator(dir, ec))
                    if (!ec &&
                        (e.path().extension() == ".png" || e.path().extension() == ".PNG"))
                        pngs.push_back(e.path());
            }
            if (pngs.empty())
                return {nullptr, {}};
        ...
        if (prof.HasFps(EnemyAnimSlot::Move))
            tc->moveFps = prof.Slot(EnemyAnimSlot::Move).fps;
        if (prof.HasFps(EnemyAnimSlot::Attack))
            tc->attackFps = prof.Slot(EnemyAnimSlot::Attack).fps;
        if (prof.HasFps(EnemyAnimSlot::Hurt))
            tc->hurtFps = prof.Slot(EnemyAnimSlot::Hurt).fps;
        if (prof.HasFps(EnemyAnimSlot::Dead))
            tc->deadFps = prof.Slot(EnemyAnimSlot::Dead).fps;
        ...
        if (Audio() && Audio()->IsReady()) {
            for (int i = 0; i < ENEMY_ANIM_SLOT_COUNT; ++i) {
                ...
                for (int fi = 0; fi < (int)slot.sfx.size(); ++fi) {
                    ...
                    if (!sfxId.empty())
                        Audio()->Sfx().LoadTrimmed(sfxId,
                                                   slot.sfx[fi].path,
                                                   slot.sfx[fi].trimStart,
                                                   slot.sfx[fi].trimEnd);
                    tc->slotSfx[i].files.push_back({slot.sfx[fi].volume,
                                                    slot.sfx[fi].timeStretch,
                                                    slot.sfx[fi].trimStart,
                                                    slot.sfx[fi].trimEnd});
                }
            }
    ...
    for (const auto& es : mLevel.enemies) {
        ...
        if (!es.enemyType.empty()) {
            ...
            if (tc && (!tc->moveFrames.empty() || !tc->idleFrames.empty())) {
                const auto& frames =
                    !tc->moveFrames.empty() ? tc->moveFrames : tc->idleFrames;
                ...
                if (tex) {
                    ...
                    reg.emplace<AnimationState>(
                        enemy, 0, (int)frames.size(), 0.0f, fps, true);
                    reg.emplace<Renderable>(
                        enemy, tex, frames, false, tc->spriteW, tc->spriteH);
                    ...
                    ead.attackSheet =
                        tc->attackSheet ? tc->attackSheet->GetTexture() : nullptr;
    ...
    if (!mBulletTex) {
        ...
        if (bs) {
            SDL_FillSurfaceRect(
                ...
                nullptr,
                SDL_MapRGBA(
                    SDL_GetPixelFormatDetails(bs->format), nullptr, 10, 10, 10, 255));
    ...
    // Build the pre-sorted tile render lists used by RenderSystem.
    {
        ...
        for (auto e : fpv)
            mSortedFrontPropList.push_back(e);
