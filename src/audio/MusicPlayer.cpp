...
// Duplicated from SoundBank.cpp to keep compilation units independent.
...
void MusicPlayer::Unload() {
    if (mTrack) {
        ...
        MIX_SetTrackAudio(mTrack, nullptr); // detach before destroying audio
    }
    if (mAudio) {
        MIX_DestroyAudio(mAudio);
        mAudio = nullptr;
...
void MusicPlayer::FadeOut(int ms) {
    if (!mTrack) return;
    // StopTrack fade_out param is in sample frames, not ms
